...
(defn deck-label
  ...
  (let [en-name (get name :en)]
    (if (= :en lang) en-name (str (get name lang) " / " en-name))))

(defn deck->autocomplete-item
  [lang [k m]]
  {:value k, :label (deck-label m lang)})

(defn view
  [{:keys [sub emit], :as ctx}]
  (let [lang (sub [:lang/code])
        items (rx/rx (mapv (partial deck->autocomplete-item @lang)
                       @(sub [:deck/all])))
