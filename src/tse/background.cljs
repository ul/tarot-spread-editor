...
(defn show-grid-menu-item
  ...
  (tse.menu/make-check-item {:content (sub [:t :menu/show-grid? "Show grid?"]),
                             ...
                             :action #(emit [:config/toggle-grid-show
...
(defn set-background-menu-item
  [{:keys [sub emit]}]
  (tse.menu/make-item {:content (sub [:t :menu/set-background
                                      "Set background"]),
...
(defn grid-css
  [grid]
  (str/format
    "repeating-linear-gradient(0deg,transparent,transparent %(step)spx,#CCC %(step)spx,#CCC calc(%(step)spx + 1px)),repeating-linear-gradient(-90deg,transparent,transparent %(step)spx,#CCC %(step)spx,#CCC calc(%(step)spx + 1px))"
    grid))
...
(defonce ^:private _resize-listener
  (do (update-body-dimensions!)
      (.addEventListener js/window "resize" update-body-dimensions!)
      true))

(defn background-width
  [node border-box-width scale]
  (if node
    (max border-box-width (/ (- @cached-body-width (.-clientLeft node)) scale))
    border-box-width))

(defn background-height
  [node border-box-height scale]
  (if node
    (max border-box-height (/ (- @cached-body-height (.-clientTop node)) scale))
    border-box-height))
        ...
        [:div
         {:ref ref-callback,
          :style {:position "absolute",
                  ...
                  :height (str (background-height @node (+ h padding) scale)
                               "px")},
          :on-click #(emit [:item/unselect-all]),
          :on-mousedown
            #(when (and (zero? (.-button %)) (.-shiftKey %))
               (let [offset [(.-offsetX %) (.-offsetY %)]
                 ...
                 (emit [:transformer/start-selection
                        (tse.math/v+ offset
                                     (tse.math/v- target-offset node-offset))
                        [(.-clientX %) (.-clientY %)]]))),
          :on-touch-start (fn [e]
