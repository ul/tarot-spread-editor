...
(defn invalidate*
  [cache v sub]
  (when (= sub (get @cache v)) (vswap! cache dissoc v)))

(defn invalidate-all*
  [cache key]
  (reduce (fn [cache [k :as v]] (if (= k key) (dissoc cache v) cache))
    cache
    (keys cache)))

(defn invalidate-all [cache key] (vswap! cache invalidate-all* key))

(defn register*
  [key->fn key f]
  (when (contains? @key->fn key) (js/console.warn "sub overwrite" key))
...
(defn make
  ...
  (let [key->fn (volatile! {})
        cache (volatile! {})]
    (letfn
      [(invalidate [v sub] (invalidate* cache v sub))
       (register [key f] (invalidate-all cache key) (register* key->fn key f))
       (register-cursor [key path]
         (register key (fn [{:keys [db]}] (rx/cursor db path))))
       (subscribe [v] (subscribe* db subscribe key->fn cache invalidate v))]
