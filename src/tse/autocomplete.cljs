(ns tse.autocomplete
  (:require [clojure.string :as str]
            [carbon.rx :as rx :refer [$] :refer-macros [$$] :include-macros
             true]
...
(defn close [{:keys [query position]} _] (reset! position 0) (reset! query ""))

(defn search
  [{:keys [query position]} [_ input]]
  (reset! position 0)
  (reset! query (str/lower-case input)))

(defn clamp [x op count] (if (pos? count) (mod (op x) count) 0))

(defn keydown
  [{:keys [position items items-count active]} [_ code]]
  (condp = code
...
(defn make-fsm
  ...
  (let [query ($ "")
    ...
    [{:query query,
      ...
      :active active}
     {:idle {:enter [close], :transitions [[(on :input :focus) :open]]},
      :open {:input [[menu-control? keydown] [(on :input) search]
...
(defn menu
  [dispatch state enum-items position good-query? hint]
  (let [position @position]
    [:.menu-wrapper {:class (when (= :open @state) "open")}
     [:.pure-menu
      [:ul.pure-menu-list
       (if @good-query?
...
(defn on-keydown
  [dispatch e]
  (let [code (.-keyCode e)]
    (when (contains? menu-keys code) (dispatch [:keydown code]))))
...
(defn view
  [{:keys [active items state placeholder hint], :or {state ($ :idle)}}]
  (let [[{:keys [query position enum-items good-query?], :as ctx} spec]
          (make-fsm active items)
        dispatch (partial fsm/execute! state spec ctx)]
    (fn [] [:form.pure-form
            {:class "autocomplete", :on-submit #(.preventDefault %)}
