                                   ...
                                   java.io.FilenameFilter
                                     (accept [_ _ filename]
                                       (boolean (re-find (re-pattern
                                                           (str "\\." fmt "$"))
                                                         filename))))))]
    (-> (clojure.java.shell/sh "identify" "-format"
                               "%w %h" (.getAbsolutePath image))
        (get :out)
...
(def edn-decks
  (vec (keep (fn [m]
               (try (let [[en ru] (clojure.string/split (get m "label")
                                                        #"\s*/\s*")
                          src (get m "value")
                          fmt (get m "ext" "jpg")
                    ...
                    (catch Exception e (println (get m "value")) nil)))
             decks)))
