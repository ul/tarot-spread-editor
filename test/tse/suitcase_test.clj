...
(deftest make-range-simple
  (is (= ["1" "2" "3"] (suitcase/make-range "1" "3"))))

(deftest make-range-zero-padded
  (is (= ["01" "02" "03"] (suitcase/make-range "01" "03"))))

(deftest make-range-single
  (is (= ["5"] (suitcase/make-range "5" "5"))))

;; parse-cards*

(deftest parse-cards-single-number
  (is (= ["01"] (suitcase/parse-cards* "01"))))
...
(deftest parse-cards-nil-input
  (is (nil? (suitcase/parse-cards* nil))))
