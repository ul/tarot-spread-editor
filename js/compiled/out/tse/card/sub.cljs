...
(defn used-by-deck [{:keys [db sub], deck :args}]
  ;; FIXME transducers ftw
  (rx/rx
   (map
    (comp vec (juxt :suit :index))
    (keep :card @(sub [:item/all])))))

(defn usable-by-deck [{:keys [db sub], [deck] :args}]
...
(defn get-entity [{:keys [sub], [id] :args}]
  (let [item (sub [:item/entity id])
    ...
    (rx/rx
     (let [{{:keys [deck suit index]} :card, :as item}  @item
